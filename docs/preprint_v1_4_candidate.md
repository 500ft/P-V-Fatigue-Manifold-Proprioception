# P-V-Triggered Recalibration of Pressure-Only Soft-Actuator Proprioception: A Simulation Study
...
already in the control loop. We study, in simulation, whether an idealized model-derived pressure–volume (P-V)
hysteresis loop can serve as a *candidate recalibration signal* of fatigue-driven proprioception degradation
...
corrector improves pose error by ≈0% at the assumed parameterization. **(2)** The dominant
...
roughly two orders of magnitude over life. **(3)** The idealized P-V loop area is correlated with that drift within the shared generator (Pearson *r* = 0.885; 95% CI [0.853, 0.950] by an exhaustive
...
than always-on. **(4)** The deployed P-V-health-triggered recalibration policy meets a train-derived budget on macro-averaged stage pose RMSE at **60% fewer recalibrations** than an always-on policy (2 vs
...
the 0.159 mm budget) at a similar recalibration count — the idealized model-derived P-V signal performs better than that cycle schedule
within this generator, not across real devices. **Scope caveat (stated up
...
**We therefore do not claim first use of P-V for fatigue.** The evaluated contribution is a five-stage simulated recalibration-policy comparison
with train-selected thresholds and a negative shared-manifold corrector result.
Study 3 does not establish noisy-probe robustness or separation of drift from varying
rest-history recovery; these require new interventions.
...
These corruptions apply to dataset observations, not the separate idealized Study 3
health signal described in §3.9.
...
The health signal is an **idealized model-derived** P-V loop area, normalized to its
young value. `scripts/run_study3.py` calls `pipeline/coupling.py::health_trajectory`,
which computes analytic SLS loop area with zero rest input to `fatigue_state`.
It does not use the noisy, quantized, decimated volume observations in the Phase D
dataset. Thus probe-noise and variable-rest robustness are not tested here. Three policies, each starting from one
...
The endpoint is the macro-average of stage-level pose RMSE across five sampled life
stages, followed by averaging across actuators (`policy_metrics` in
`scripts/run_study3.py`). It is not a maximum error or a guarantee between stages.
Against a train-derived 0.159 mm budget (selected on training actuators as halfway from the
...
| Policy | Macro-averaged stage pose RMSE (held-out) | Calibration events / actuator, including initial | Meets mean 0.159 mm budget? |
|---|---:|---:|---|
...
- **Correlation is structurally favored, not demonstrated diagnostic robustness.**
  ...
  uses zero rest input. Those mechanisms cannot explain its correlation interval.
  Pose-channel noise and estimator/PCC behavior affect the other variable, but
  are not a substitute for health-signal perturbations. Young-normalization makes
  the health trajectory effectively identical across actuators. The intervals
  describe resampling uncertainty among six synthetic identities, not robustness
  to alternative failure modes or real devices. Leak-dominated, sensor-biased
  and local-damage interventions must be tested separately.
- **The headline cross-talk hypothesis was not supported.** Cross-talk is monotone in compliance
  (Gate 0) but second-order for pose at the tested parameters. The proposal anticipated this
...
- **Parameter-sensitivity of cross-talk (envelope, Fig. 5).** To make the "second-order" claim a
  ...
  the negative result is not knife-edge: cross-talk remains second-order across the tested
  model configurations and would only become first-order for pose under a deliberately
...
P-V-triggered policy meets the train-derived macro-averaged stage-RMSE budget
with fewer counted calibration events (including initialization) than always-on
